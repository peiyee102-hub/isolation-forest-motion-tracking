...
def run_calibration(ser, session, logger):
    ...
    while accepted < N_CALIB_REPS:
        ...
        while time.time() - t_start < CALIB_DURATION:
            ...
            sys.stdout.write(
                f"\r  {time_bar(elapsed, CALIB_DURATION)}  "
                f"Angle: {angle_bar(angle, 0, 130)}  Peak: {live_max:5.1f}°"
            )
            sys.stdout.flush()
        print()
        ...
        with ser:
            ...
            while True:
                ...
                while time.time() - t_start < REP_WINDOW_SEC:
                    ...
                    # Continuous live feedback during the 9s window
                    sys.stdout.write(
                        f"\r  {time_bar(elapsed, REP_WINDOW_SEC)}  "
                        f"Angle: {angle_bar(angle, 0, calib_max)}"
                    )
                    sys.stdout.flush()
